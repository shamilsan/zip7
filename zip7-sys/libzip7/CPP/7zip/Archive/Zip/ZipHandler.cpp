...
namespace NArchive
{
  namespace NZip
  {
    ...
    static const char *const kHostOS[] =
        {
            "FAT", "AMIGA", "VMS", "Unix", "VM/CMS", "Atari", "HPFS", "Macintosh", "Z-System", "CP/M", "TOPS-20", "NTFS", "SMS/QDOS", "Acorn", "VFAT", "MVS", "BeOS", "Tandem", "OS/400", "OS/X"};
    ...
    const char *const kMethodNames2[kNumMethodNames2] =
        {
            "zstd", "MP3", "xz", "Jpeg", "WavPack", "PPMd", "LZFSE" // , "WzAES"
    };
...
#define kMethod_StrongCrypto "StrongCrypto"
        ...
        Z7_COM7F_IMF(CHandler::GetArchiveProperty(PROPID propID, PROPVARIANT *value))
    {
      ...
      case kpidBit64:
        if (m_Archive.IsZip64)
          prop = m_Archive.IsZip64;
        break;
      case kpidComment:
        if (m_Archive.ArcInfo.Comment.Size() != 0)
          prop = MultiByteToUnicodeString(BytesToString(m_Archive.ArcInfo.Comment), CP_ACP);
        break;

      case kpidPhySize:
        prop = m_Archive.GetPhySize();
        break;
      case kpidOffset:
        prop = m_Archive.GetOffset();
        break;
      ...
      case kpidTotalPhySize:
        if (m_Archive.IsMultiVol)
          prop = m_Archive.Vols.TotalBytesSize;
        break;
      ...
      case kpidNumVolumes:
        if (m_Archive.IsMultiVol)
          prop = (UInt32)m_Archive.Vols.Streams.Size();
        break;
      ...
      case kpidErrorFlags:
      {
        ...
        if (m_Archive.HeadersError)
          v |= kpv_ErrorFlags_HeadersError;
        if (m_Archive.UnexpectedEnd)
        ...
        if (m_Archive.NoCentralDir)
          v |= kpv_ErrorFlags_UnconfirmedStart;
        prop = v;
        break;
      }
    ...
    */
    ...
    Z7_CLASS_IMP_NOQIB_3(
        CLzmaDecoder, ICompressCoder, ICompressSetFinishMode, ICompressGetInStreamProcessedSize)
  public:
  ...
  public:
    CZipDecoder() : lzmaDecoderSpec(NULL)
    {
    }

    HRESULT Decode(
        DECL_EXTERNAL_CODECS_LOC_VARS CInArchive &archive, const CItemEx &item,
  ...
  Z7_CLASS_IMP_NOQIB_1(
      COutStreamWithPadPKCS7, ISequentialOutStream)
    ...
    Int32 &res)
{
  ...
  const bool useUnpackLimit = (id == NFileHeader::NCompressionMethod::kStore || !item.HasDescriptor() || item.Size >= ((UInt64)1 << 32) || item.LocalExtra.IsZip64 || item.CentralExtra.IsZip64);

  {
    ...
    if (result == S_OK)
    {
      ...
      if (id == NFileHeader::NCompressionMethod::kStore && item.IsEncrypted())
      {
        ...
        result = filterStream->Code(inStream, padStream ? padStream.Interface() : outStream.Interface(),
      ...
      else
      {
        ...
        try
        {
          result = coder->Code(readFromFilter ? filterStream.Interface() : inStream.Interface(),
                               outStream,
  ...
  if (wzAesMode)
  {
    ...
    if (SkipStreamData(readFromFilter ? filterStream.Interface() : inStream.Interface(),
                               ...
                               Int32 testMode, IArchiveExtractCallback *extractCallback))
{
      ...
      lps->InSize += cur_Packed)
  {
    ...
    const Int32 askMode = skip ? NExtract::NAskMode::kSkip : testMode ? NExtract::NAskMode::kTest
    ...
    Int32 opRes;
    {
        ...
        if (hres == S_FALSE)
        {
          if (item.IsDir() || realOutStream || testMode)
          {
            ...
            RINOK(extractCallback->SetOperationResult(
                isAvail ? NExtract::NOperationResult::kHeadersError : NExtract::NOperationResult::kUnavailable))
          }
          continue;
        }
        RINOK(hres)
      ...
      const HRESULT hres = myDecoder.Decode(
          EXTERNAL_CODECS_VARS
              m_Archive,
          item, realOutStream, extractCallback,
          lps,
#ifndef Z7_ST
