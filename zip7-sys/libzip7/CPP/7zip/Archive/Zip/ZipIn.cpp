    ...
    */
    ...
    HRESULT CInArchive::ReadFromCache(Byte *data, unsigned size, unsigned &processed)
    {
      ...
      for (;;)
      {
        ...
        if (!IsMultiVol || !CanStartNewVol || Vols.StreamIndex < 0 || (unsigned)Vols.StreamIndex + 1 >= Vols.Streams.Size())
          break;
      ...
      if (extraSize > 0)
      {
        ...
        while (extraSize != 0)
        {
          ...
          if (dataSize > extraSize)
          {
            ...
            if (nameSize == 0 || nameSize > (1 << 9) || extraSize > (1 << 9))
              return k_IsArc_Res_NO;
            return k_IsArc_Res_YES;
          }
          if (dataSize > size)
    ...
    HRESULT CInArchive::FindMarker(const UInt64 *searchLimit)
    {
      ...
      for (;;)
      {
        ...
        if (isFinished)
        {
          ...
          {
            if (!IsMultiVol || Vols.StreamIndex < 0 || (unsigned)Vols.StreamIndex + 1 >= Vols.Streams.Size())
              break;
        ...
        for (;; p++)
        {
          ...
          if (res != k_IsArc_Res_NO)
          {
            ...
            ArcInfo.MarkerPos2 = ArcInfo.MarkerPos;
            SkipLookahed(4);
            if (_signature == NSignature::kNoSpan || _signature == NSignature::kSpan)
            {
...
#define ZIP64_IS_16_MAX(n) ((n) == 0xFFFF)
                               ...
                               CItem *cdItem)
    {
      ...
      while (extraSize >= 4)
      {
        ...
        if (subBlock.ID == NFileHeader::NExtraID::kZip64)
        {
          ...
          if (!cdItem && size == 16 && !ZIP64_IS_32_MAX(unpackSize) && !ZIP64_IS_32_MAX(packSize))
          {
            ...
            if (cdItem)
            {
              if (isOK)
              {
                if (ZIP64_IS_32_MAX(cdItem->LocalHeaderPos))
                {
                  ...
                  {
                    size -= 8;
                    cdItem->LocalHeaderPos = ReadUInt64();
                  }
                }
                /*
                else if (size == 8)
    ...
    static bool FlagsAreSame(const CItem &i1, const CItem &i2_cd)
    {
      ...
      if (mask & NFileHeader::NFlags::kUtf8)
        if ((i1.Name.IsAscii() && i2_cd.Name.IsAscii()) || (i2_cd.Flags & NFileHeader::NFlags::kUtf8))
    ...
    static bool AreItemsEqual(const CItemEx &localItem, const CItemEx &cdItem)
    {
      ...
      {
        if (cdItem.PackSize != localItem.PackSize || cdItem.Size != localItem.Size || (cdItem.Crc != localItem.Crc && cdItem.Crc != 0)) // some program writes 0 to crc field in central directory
          return false;
      }
    ...
    */
    ...
    HRESULT CInArchive::FindDescriptor(CItemEx &item, unsigned numFiles)
    {
      ...
      for (;;)
      {
        ...
        if (descriptorSize4 > Buffer.Size())
          return E_FAIL;
      ...
      for (size_t i = bufSize - kEcdSize + 1;;)
      {
        ...
        if (i >= kEcd64Locator_Size)
        {
          ...
          if (Get32(buf + locatorIndex) == NSignature::kEcd64Locator)
          {
            ...
            if (numDisks == 0)
              numDisks = 1;
            if ((cdInfo.ThisDisk == numDisks - 1 || ZIP64_IS_16_MAX(cdInfo.ThisDisk)) && locator.Ecd64Disk < numDisks)
            {
        ...
        CanStartNewVol = false;
        {
          ...
          */
          ...
          if (items.Size() > 0 && !IsCdUnsorted)
          {
            ...
            if (cdItem.Disk < prev.Disk || (cdItem.Disk == prev.Disk &&
      ...
      // if (Vols.StartVolIndex >= 0)
      {
        if (Vols.Streams.IsEmpty())
          if (Vols.StartVolIndex > (1 << 20))
            return S_OK;
        if ((unsigned)Vols.StartVolIndex >= Vols.Streams.Size() || !Vols.Streams[(unsigned)Vols.StartVolIndex].Stream)
        {
...
#define COPY_ECD_ITEM_16(n)                \
  if (!isZip64 || !ZIP64_IS_16_MAX(ecd.n)) \
    cdInfo.n = ecd.n;
#define COPY_ECD_ITEM_32(n)                \
  if (!isZip64 || !ZIP64_IS_32_MAX(ecd.n)) \
    cdInfo.n = ecd.n;
              ...
              else
              {
                ...
                else
                {
                  FOR_VECTOR(i, items)
                  {
                    ...
                    if (cdItem.Disk == firstItem.Disk && (cdItem.LocalHeaderPos == firstItem.LocalHeaderPos))
                      index = (int)i;

                    if (i == 0 || cdItem.Disk < min_Disk || (cdItem.Disk == min_Disk && cdItem.LocalHeaderPos < min_LocalHeaderPos))
                    {
          ...
          if (nextLocalIndex != -1)
          {
            if ((unsigned)nextLocalIndex < items.Size())
            {
              ...
              if (item.Disk == cdItem.Disk &&
                  (item.LocalHeaderPos == cdItem.LocalHeaderPos || (Overflow32bit && (UInt32)item.LocalHeaderPos == cdItem.LocalHeaderPos)))
                             ...
                             IArchiveOpenCallback *callback, CObjectVector<CItemEx> &items)
    {
      ...
      else if (IsMultiVol && Vols.StartParsingVol == 0 && (unsigned)Vols.StartParsingVol < Vols.Streams.Size())
      {
        LOG("5")
      ...
      else
      {
        LOG("6")
        // printf("\nOpen offset = %u\n", (unsigned)startPos);
        if (IsMultiVol && (unsigned)Vols.StartParsingVol < Vols.Streams.Size() && Vols.Streams[(unsigned)Vols.StartParsingVol].Stream)
        {
